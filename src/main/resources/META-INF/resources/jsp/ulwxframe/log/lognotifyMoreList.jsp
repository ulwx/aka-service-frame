    ...
    <script type="text/javascript">
        ...
        function getDataGridColums() {
            var columns=[[
                ...
                {field: 'className', title: '接口名称', align: 'center',width:120,sortable:true},
        ...
        function opens(index,num){
            var rows=$(".lognotifyMoreList").datagrid("getRows");
            var row=rows[index];
            if(num == 1){
        ...
        $(document).ready(function () {
            $("#dlg2").dialog('close');
            var gridUrl = "<%=request.getContextPath()%>/frame/log_Log_getLogNotifyMoreListJSON.action";
            initDataGrid(".lognotifyMoreList", gridUrl, getQueryParm(), getDataGridColums(), {fitColumns: false});
            $("body").css("visibility", "visible");
        });

        function lookup() {
            $('.lognotifyMoreList').datagrid("load", getQueryParm());
        }
        ...
        function reloadGrid() {
            $(".lognotifyMoreList").datagrid("reload");
        }
    </script>
</head>
...
<div region="center" border="false" title="" data-options="headerCls:'headerClass'">
    ...
    <table id="lognotifyMoreList" class="lognotifyMoreList">
    </table>
</div>
</body>
</html>